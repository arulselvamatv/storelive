...
if($con)
{
    ...
    WHERE wo.`active_record` =1 and s.`active_record` =1 and st.dep!=0  and history=1 group by  t.t_id ORDER BY t.t_id');
   $i=0;
    ...
    { 
        ...
        {
             echo'<td>Back To Store'.
            '<input type="hidden" name="sup[]" id="sup_'.$i.'" value="'.$prd['Back To Store'].'"/>'.
            '</td>';
        ...
        if($prd['todep']==0)
        {
            ...
            <td>'. '<input type="hidden" name="dep[]" id="dep_'.$i.'" value="'.$prd['todep'].'"/ >'. '</td>';
