...
$HTML="";
...

	{
		...
		$asd =('SELECT `qas_id`,`bill_type`, `wo_no`, quo_no ,date_time FROM `quot_sup_amt_sel` 

		WHERE `active_record`=1  group by wo_no order by date_time desc ');
		...
		{ 
			...
			'<td>'.
			'<input class="form-control" name="bill_type[]" id="bill_type'.$i.'" value="'.$prd['bill_type'].'" readonly="readonly" />'.
			'</td>'.

            '<td><input class="form-control" name="date_time[]" id="date_time_'.$i.'" value="'.$prd['date_time'].'" readonly="readonly" />'.

			'</td>'.

			'<td><center><input id="edit_'.$i.'" name="edit[]" class="btn btn-primary edit" type="button" value="Edit"  readonly="readonly"/></center>'.

			'</td>'.

			'<td><a href=work_order_view.php?wo_no="'.$prd[wo_no].'" target="_blank"><div class="inner"><input class="btn btn-primary editz w-75" name="edit-button[]"  value="View" style="display:block"  readonly="readonly"/></div></a>'.
			'</td>'.
