...
$HTML="";
	if($con)
	{
	    $HTML=$HTML. '<form action="finance_back.php" method="post">';
		...
		count(st.`store_id`)  as received_qty, st.`billno`, st.`bill_date`, st.`bill_amt`as bill_amt, st.`received_date` as po_date, st.`u_id`,wo.suplier_id as suplier_id,
	
		s.name as supplier_name,s.company_name,s.address,wo.quo_no as quo_no,(wo.prod_name) as item_name,wo.ddl_pro_qty,wo.product_spec as item_desc,wo.suplier_id, st.se_no as se_no ,
		
		wo.po_no as po_no,sum(st.grand_total) as grand_total FROM `po_entry` as st 
        ...
        WHERE wo.`active_record` =1 and s.`active_record` =1 and wo.dep_no="'.$po_id.'"  group by wo.dep_no');
         ...
         //echo $asda; exit;
        	...
        	while($rows = $dataa->fetch(PDO::FETCH_ASSOC))
        	{
                    	...
                    	{ 
                    	   if($rowsss['grb_edit_history'] != 0)
                    	    {
                    	 $html2=$html2.'
                    	 <span style="background-color:#094e6c;color:#fff;">
                    ...
                    //      $html2=$html2." ";
                        ...
                        <tr> <td><input  type="radio" checked value="Advance" id="chk_Advance" name="grb_payment_for" >Advance Payment  &nbsp;&nbsp;&nbsp; 
                                                                                 <input  type="radio" checked value="Payment" id="chk_Payment" name="grb_payment_for" >Payment</td>
			...
			'
				...
				Amount Paid on Work Order:</td><td align="left"> Rs.'.$advance.'</td></tr> <br>
			';
        ...
        // 	echo $asdaaa; exit;
			...
			'
				...
				Amount Paid:</td><td align="left" width="16%"> Rs.'.$rowsaaa['payment_amount'].'</td><td width="16%">Payment Mode.</td><td align="left" width="16%">'.$rowsaaa['payment_mode'].'</td><td width="16%">Transaction No.</td><td align="left" width="16%">'.$rowsaaa['transaction_no'].'</td></tr>
			';
			$tot=$tot+$rowsaa['payment_amount'];
				...
				<div class="panel panel-default">
				<div class="panel-heading"><strong>Payment History:</strong></div><br>
				<div class="panel-body">
				<table border=1 width="100%">
				';
...
// 			$tot=$tot+$rowsaa['payment_amount'];
		...
		$HTML=$HTML.
					...
					<div class="form-group">
						<label class="control-label col-sm-4">Enter the Amount Paid:</label>
							...
							<input type="hidden" name="inv_no" value="'.$po_id.'" >
								<input class="form-control" type="text"  name="payment_amount" style="width:300px;border:1px solid gray"><br> <font color="red">(Payment Already Made Rs.'.($tot+$advance).')</font>
							</div>
					</div>
				</div>
				<div class="col-sm-6">
			...
			<div class="col-sm-6">
				<div class="form-group">
					<label class="control-label col-sm-6">Reference No./Cheque No.</label>
					<div class="col-sm-9">
