...
//return  $insert;
	   ...
	   public function insert_new_return_id($table,$rows)
    {
	   ...
	   public function insertMultiple($table,$values,$rows = null) {
	            //~ echo "<pre/>rdata";print_r($values);exit;
	        $insert = 'INSERT INTO '.$table;
            if($rows != null)  {
                $insert .= ' ('.$rows.')';
            }
			if (count($values) == count($values, COUNT_RECURSIVE)){
				$array_values[0] = $values; 
			...
			for($j = 0; $j < count($array_values); $j++){
				...
				for($i = 0; $i < count($array_values[$j]); $i++){
					if(is_string($array_values[$j][$i])&&$array_values[$j][$i]!='NULL')
						$array_values[$j][$i] = "'".$array_values[$j][$i]."'";
					elseif($array_values[$j][$i]=='NULL')
						$array_values[$j][$i] = $array_values[$j][$i];	
				}
				$updated_array_values = implode(',',$array_values[$j]);
				$insert .= '('.$updated_array_values.')';
				if($j == count($array_values) - 1){
					$insert .= ';';
				}else if( count($array_values) >0){
					$insert .= ',';
				}
			}
			//~ echo "<pre/>insert";print_r($insert);exit;
			try {  
				 $stmt = $this->conn->prepare($insert);
		        $stmt->execute();
				   return  $this->conn->lastInsertId($table);
