...
$HTML="";
	if($con)
	{
		...
		<div style=\"border:1px solid black; margin:5px;\"><table><tr><td>";
		$html2=$html2.'
			&nbsp;&nbsp;&nbsp;&nbsp;<input  type="radio" checked value="Advance" id="chk_Advance" name="grb" >Advance Payment';

            
            
		$asda =('SELECT wo.dep_no as invoice_no,st.`store_id`, st.code, st.`po_id`, st.`p_row_id`, st.`actual_qty`, st.`received_qty` as qty, st.`per_amt` as `per_amt`, count(st.`store_id`)  as received_qty, st.`billno`, st.`bill_date`, st.`bill_amt`as bill_amt, st.`received_date` as po_date, st.`u_id`,wo.suplier_id as suplier_id, s.name as supplier_name,s.company_name,s.address,wo.quo_no as quo_no,(wo.prod_name) as item_name,wo.ddl_pro_qty,wo.product_spec as item_desc,wo.suplier_id, st.se_no as se_no , wo.po_no as po_no,st.grand_total FROM `po_entry` as st 
        ...
        WHERE wo.`active_record` =1 and s.`active_record` =1 and wo.dep_no="'.$po_id.'"  group by wo.dep_no');
         //echo $asda; exit;
        	...
        	while($rows = $dataa->fetch(PDO::FETCH_ASSOC))
        	{
                        ...
                        $html2=$html2.'</td></tr></table></div><br/>';
                        $date=date("d-m-Y",strtotime($rows['date_time']));
			...
			'
				...
				Amount Paid on WO:</td><td align="left"> Rs.'.$advance.'</td></tr>
			';
		...
		$HTML=$HTML.
					...
					<div class="form-group">
						<label class="control-label col-sm-4">Enter the Amount Paid:</label>
							...
							<input type="hidden" name="inv_no" value="'.$po_id.'" >
								<input class="form-control" type="text"  name="payment_amount"> <font color="red">(Payment Already Made Rs.'.($tot+$advance).')</font>
							</div>
					</div>
				</div>
				<div class="col-sm-6">
			...
			<div class="col-sm-6">
				<div class="form-group">
					...
					<div class="col-sm-9">
						<textarea class="form-control" name="transaction_no" rows=3></textarea>
					</div>
				</div>
			</div>
			</div>
