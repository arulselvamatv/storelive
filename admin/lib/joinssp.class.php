...
class JOINSSP {
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = JOINSSP::pluck( $columns, 'dt' );
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = JOINSSP::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
     ...
     * in response to an JOINSSP request, or can be modified if needed before
    ...
    {
        $bindings = array();
        $db = JOINSSP::sql_connect( $sql_details );

        // Build the SQL query string from the request
        $limit = JOINSSP::limit( $request, $columns );
        $order = JOINSSP::order( $request, $columns, $joinQuery );
        $where = JOINSSP::filter( $request, $columns, $bindings, $joinQuery );
        ...
        if($joinQuery){
            $col = JOINSSP::pluck($columns, 'db', $joinQuery);
       ...
       $having
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", JOINSSP::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
       $having
        ...
        $data = JOINSSP::sql_exec( $db, $bindings,$query);

        // Data set length after filtering
        $resFilterLength = JOINSSP::sql_exec( $db, "SELECT FOUND_ROWS()");
        $recordsFiltered = $resFilterLength[0][0];

        // Total data set length
        $resTotalLength = JOINSSP::sql_exec( $db, "SELECT COUNT(`{$primaryKey}`) FROM   `$table`");
        ...
        return array(
            ...
            "data"            => JOINSSP::data_output( $columns, $data, $joinQuery )
        );
    }
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
            ...
            JOINSSP::fatal( "An SQL error occurred: ".$e->getMessage() );
        }
