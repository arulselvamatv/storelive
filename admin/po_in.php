				...
				<div class="col-sm-12">
					<div class="card"></br>
                        ...
                        <div class="card-header bg-info">
                            <h4 class="mb-0 text-white">GRB IN Entry</h4>
                        </div></br>
                        </br>
                        <div id="update_div">
  						<div class="form-group px-4 py-5">
							<label class="control-label col-sm-4"><big><font color="red">GRB IN Entry</font></big> :: Enter Department Number</label>
							<div class="col-sm-6">
								<select name="po_no" id="po_no" class="js-example-basic-multiple" style="width:100%;" onkeypress="view(event);" > 
								<option value="">Select Dept No</option>
									                        <?php
                                                            ...
                                                            //echo $asd; exit;
                                                            $asd =('SELECT wo.`dep_no`,wo.`po_no` FROM `work_order1` as wo WHERE `active_record`=1 AND (SELECT SUM(`ddl_pro_qty` - `po_status`) as nodata FROM `work_order1` INNER JOIN suplier AS s ON s.sup_id = work_order1.suplier_id WHERE work_order1.po_no=wo.po_no AND work_order1.`active_record`=1) > 0 GROUP BY `current_date_time` desc');
                                                            $datd = $db->query($asd);
...
<script>
    ...
    $(document).on('change', '#po_no', function( e ) {
		...
		$.ajax({
		type: "GET",
		url: 'ajax/get_po_entry.php?val='+$("#po_no").val(),
