...
//echo "<hr/>";
//header over
    $asd =('SELECT s.name,s.mobile,s.company_name, s.address,`wo_id`, dep_no, dept,location, `po_no`, `wo_no`, `quo_no`, `prod_name`, `ddl_pro_unit`, `ddl_pro_qty`, `product_spec`, `suplier_id`, `sup_amt`, `disc_amt`, `gst_amt`, `tot`, work_order1.`active_record`, work_order1.`date_time` FROM `work_order1` 
...
while($row = $data->fetch(PDO::FETCH_ASSOC))
   ...
   while($depts = $dept_1->fetch(PDO::FETCH_ASSOC))
    ...
    if($i==0)
    {
        ...
        echo"<div style=\"text-align:right;\"> <b>Order No:</b>".$row['dep_no']."<b> / Dated: ".date("d-m-Y H:m:s",strtotime($row['date_time']))."</b></div>
...
<table width="100%">
    ...
    if($postatus && (strtolower($postatus) == 'approved')){
        echo '<img style=text-align:right;width:200px;height:81px src="../admin/assets/Sign.png">';
    }else{
        echo "</br>";
    }
    ?>
