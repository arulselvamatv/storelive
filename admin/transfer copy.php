                    ...
                    <div class="col-lg-12">
                        <div class="card"></br>
                            ...
                            <div class="card-header bg-info">
                                <h4 class="mb-0 text-white">Transfer</h4>
                            </div>
                                ...
                                $db=new Database;
                                $db->connect();
                                $asd =('SELECT c.dep_name,st.`store_id`, st.code, st.invoice_no, st.wrt_date,s.company_name, st.`po_id`, st.`p_row_id`, st.`actual_qty`, st.`received_qty` as qty, st.`per_amt` as `per_amt`, count(st.`store_id`)  as received_qty, st.`billno`, st.`bill_date`, st.`bill_amt`as bill_amt, st.`received_date` as po_date, st.`u_id`, st.`grb_id` as grb_id, g.overall_total as overall_total,wo.suplier_id as suplier_id, s.name as supplier_name,wo.quo_no as quo_no,(wo.prod_name) as item_name,wo.ddl_pro_qty,wo.product_spec as item_desc,wo.suplier_id, st.se_no as se_no , wo.po_no as po_no FROM `store_entry` as st 
                                ...
                                WHERE wo.`active_record` =1 and s.`active_record` =1 and st.dep!=0 and st.tranf = 0  group by st.se_no');
                                //echo $asd; exit;
                                $datd = $db->query($asd);
                                ...
                                while($row = $datd->fetch(PDO::FETCH_ASSOC))
                                {
                                  ...
                                  echo "<td>".$row['wrt_date']."</td>";
                                  echo "<td><div id='outer'>
                                  <center>
                                  <a href='?se_no=".$row['se_no']."'><div class='inner'><input class='btn btn-primary editz' name='edit-button[]'  value='Transfer'/></div></a>
                                  </center>
                                  </div></td>";
