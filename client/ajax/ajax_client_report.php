...
$po_no = $_GET['po_no'];
$clientid = $_SESSION['user_log_id'];

$HTML = "";
if ($con) {
    $asd = ("SELECT `cl_id`, `client`.`dep_name`, `room_no`, `block`, `date_time`, `active_record` FROM `client` JOIN clientusers ON client.cl_id=clientusers.dep_name WHERE `clientusers`.`id`='".$clientid."' AND `active_record`=1");
    $e = 0;
